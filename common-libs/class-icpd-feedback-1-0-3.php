...
if ( ! class_exists( 'ICPD_Feedback' ) ) {
	final class ICPD_Feedback {
		...
		public function print_view_html_feedback_block() { ?>
			<div class="postbox">
				<h2 class="hndle">
					<?php esc_html_e( 'Send data about the work of the plugin', 'yml-for-yandex-market' ); ?>
				</h2>
				<div class="inside">
					<?php
					printf( '<p>%s! %s:</p>',
						esc_html__( 'Sending statistics you help make the plugin even better', 'yml-for-yandex-market' ),
						esc_html__( 'The following data will be sent', 'yml-for-yandex-market' )
					);
					?>
					<ul class="icpd_ul">
						<?php
						printf( '<li>%s</li><li>%s</li><li>%s %s</li></p>',
							esc_html__( 'PHP version information', 'yml-for-yandex-market' ),
							esc_html__( 'Multisite mode status', 'yml-for-yandex-market' ),
							esc_html__( 'Technical information and plugin logs', 'yml-for-yandex-market' ),
					...
					<form action="<?php echo esc_url( $_SERVER['REQUEST_URI'] ); ?>" method="post" enctype="multipart/form-data">
						<?php
						printf( '<p>%s %s?</p>',
							esc_html__( 'Did my plugin help you upload your products to the', 'yml-for-yandex-market' ),
							esc_html( $this->get_plugin_name() )
						);
						?>
						<p>
							...
							printf( '<input type="radio" value="yes" name="%s">%s<br />',
								...
								esc_html__( 'Yes', 'yml-for-yandex-market' )
							);

							printf( '<input type="radio" value="no" name="%s">%s<br />',
								esc_attr( $this->get_radio_name() ),
								esc_html__( 'No', 'yml-for-yandex-market' )
							);
							?>
						</p>
						<p>
						...
						<p>
							<?php esc_html_e( 'Your message', 'yml-for-yandex-market' ); ?>:
						</p>
						<p><textarea rows="6" cols="32" name="<?php echo $this->get_textarea_name(); ?>" placeholder="<?php
						   printf( '%1$s (%2$s). %3$s',
						   	esc_attr( esc_html__( 'Enter your text to send me a message', 'yml-for-yandex-market' ) ),
						   	esc_attr( esc_html__( 'You can write me in Russian or English', 'yml-for-yandex-market' ) ),
						   	esc_attr( esc_html__( 'I check my email several times a day', 'yml-for-yandex-market' ) )
						   ); ?>"></textarea></p>
